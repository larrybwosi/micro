...
fn test_schedule_calculations() {
    let flat_schedule = calculate_loan_schedule(1000.0, 12.0, "ANNUAL", 12, "FLAT_RATE", "2026-01-01");
    ...
    // Monthly rate test
    let monthly_flat_schedule = calculate_loan_schedule(1000.0, 1.0, "MONTHLY", 12, "FLAT_RATE", "2026-01-01");
    ...
    let emi_schedule = calculate_loan_schedule(1000.0, 12.0, "ANNUAL", 12, "REDUCING_BALANCE", "2026-01-01");
    ...
    let bullet_schedule = calculate_loan_schedule(1000.0, 12.0, "ANNUAL", 12, "INTEREST_ONLY", "2026-01-01");
...
fn test_expenses_and_petty_cash() {
    ...
    let approved_exp = get_all_expenses(&conn).unwrap().into_iter().find(|e| e.id == Some(exp2_id)).unwrap();
...
fn test_edge_case_calculations() {
    // 0 principal or term
    let zero_principal = calculate_loan_schedule(0.0, 12.0, "ANNUAL", 12, "FLAT_RATE", "2026-01-01");
    assert!(zero_principal.is_empty());

    let zero_term = calculate_loan_schedule(1000.0, 12.0, "ANNUAL", 0, "FLAT_RATE", "2026-01-01");
    assert!(zero_term.is_empty());

    // 0 interest rate
    let zero_rate = calculate_loan_schedule(1200.0, 0.0, "ANNUAL", 12, "REDUCING_BALANCE", "2026-01-01");
